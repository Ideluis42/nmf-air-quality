...
mod_met$date <- as.POSIXct(strptime(mod_met$timestamp_local.x, format = "%Y-%m-%d %H:%M:%S", tz = "US/Central"))
```

```{r}
# drop na columns

na_cols <- c("sitename", "original_met_time", "timestamp_local.y")
...
```{r}
save(aq_df_filtered, file = paste0("\\\\wsl.localhost/Ubuntu/home/ideluis42/nmf-air-quality/Galena_Park/", "cleaned_modpm.RData"))
```
